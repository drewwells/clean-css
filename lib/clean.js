  ...
  process: function(data, options) {
    ...
    replace(function stripComments() {
      ...
      for (; nextEnd < data.length; ) {
        ...
        if (data[nextStart + 2] == '!') {
          ...
          specialComments.push(data.substring(nextStart, nextEnd + 2));
      ...
      if (tempData.length > 0) {
        tempData.push(data.substring(cursor, data.length));
        data = tempData.join('');
      }
    });
    ...
    replace(function stripContent() {
      ...
      for (; nextEnd < data.length; ) {
        ...
        contentBlocks.push(data.substring(nextStart - 1, nextEnd + 1));
        cursor = nextEnd + 1;
      }
